# **Banner**
Welcome to Gatekeeper's Display Banners page. In this section we will cover how to use and interact with Gatekeeper's Banners.
...
___
Most of the settings will be handled via [Server Setting Commands](/docs/COMMANDS.md#amp-server-settings-commands); these usually include your `Host`, `Whitelist` and `Donator`.  
...
- **TIP**: Use `Blur Background Intensity` to help with text readability by blurring the background. Default is `0`

- Below you will see an example picture of what Gatekeeper's `Banner Editor` looks like.
...
- Here is what `Server Online Font Color` will affect when the server is Online, or `Server Offline Font Color` when it is Offline.  
    -  `Player Limit Minimum Font Color` and `Player Limit Maximum Font Color` are your min and max color hex values.
...
___
Cool, you have probably figured out how to edit your banners and make them look how you want; but you still can't see them anywhere yet. Let's start!
...
1. First we need to create a `Banner Group` use `/bot bannergroup create_group` and enter a group name *(eg. Minecraft)*
    - The name you enter is for identification purposes only. Autocomplete will prompt all group names.
2. Next, use `/bot bannergroup add` and select the group you recently made in step 1, then select the `server` parameter and choose a server from the autocomplete to add it to the group.
