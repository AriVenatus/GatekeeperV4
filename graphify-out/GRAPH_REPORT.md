...
## Summary
- 1338 nodes · 2621 edges · 86 communities (58 shown, 28 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 78 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d1fe6e22`
...
Cohesion: 0.08
Nodes (28): before_loop, autocomplete, Bot, Choice, choices, command, Context, describe (+20 more)

### Community 5 - "DB.py"
Cohesion: 0.25
Nodes (8): setup(), get_language(), getI18nHandler(), # NOTE: `Command._params` is a private/undocumented discord.py attribute…, retranslate_command_tree(), set_language(), t_plural(), # NOTE: these field NAMES stay English-only by design -- they're auto-titled…
...
### Community 7 - ".Login"
Cohesion: 0.08
Nodes (14): This is the main API Call function, This gets all Instances on AMP., Restarts AMP Instance, Returns a List of connected users., This is used to change an Instance's Friendly Name and or Description. Retains…, Test AMP API calls with this function, Ends specified User Session, Returns currently active AMP Sessions (+6 more)
...
### Community 11 - "AMP.py"
Cohesion: 0.19
Nodes (4): AMPCsgo, AMPCsgoConsole, AMPValheim, AMPValheimConsole
...
Cohesion: 0.09
Nodes (12): AMPInstance, Base Function for AMP.addWhitelist, Base Function for AMP.getWhitelist, Base Function for AMP.removeWhitelist, Base Function for AMP.name_Conversion, Base Function for AMP.name_History, Base Function for Discord Chat Messages to AMP ADS, Base Function for Server Chat Message Formatter (+4 more)
...
Cohesion: 0.09
Nodes (12): Removes a entry RegexPatterns Table using either its `Name` or `ID`, Returns RegexPatterns Table Returns `row['ID'] = {'Name': row['Name'], 'Type':…, Update a Regex Pattern in the RegexPatterns Table using either its `Name` or…, Selects a Banner Group Table matching the `name` provided., Update a Banner Group, This sets the DB Server Console_Flag, Console_Filtered and…, Add a Server to an existing Banner Group., Removes a Server from an existing Banner Group. (+4 more)
...
Cohesion: 0.15
Nodes (13): botEmbeds, Bot, Context, Embed, User, This is the Server Status Embed Message, Gatekeeper Embeds/Banners, Default Embed Reply for Successful Whitelist requests (+5 more)
...
Cohesion: 0.05
Nodes (58): AMP_init(), amp_server_instance_check(), AMPHandler, getAMPHandler(), Intializes the connection to AMP and creates AMP_Instance objects., Creates a list of Instance Names/DisplayName or Friendly Name., Builds a tokens-like namespace from environment variables (optionally loaded…, Validates the .env/environment-variable settings and 2FA. (+50 more)

### Community 30 - ".__init__"
Cohesion: 0.16
Nodes (7): AMPHandler, Creates the `Gatekeeper` role, Adds us to the Membership of that Role and sets…, Sets the Permissions Nodes for AMP Gatekeeper Role, This is used to set/update the DB attributes for the AMP server, Creates a AMP User role, Sets the AMP Users Role Membership, Sets the AMP Role permission Node eg `Core.RoleManagement.DeleteRoles`
...
Cohesion: 0.33
Nodes (4): datetime, This will be used to access channel history up to 100. Simple scraper with…, This is the bot utils Channel Parse Function It handles finding the specificed…, TextChannel
...
## Knowledge Gaps
- **96 isolated node(s):** `**Using your commands!**`, `<u>Bot Commands</u>:`, `<u>Bot Utils Commands</u>:`, `<u>Bot Cog Commands</u>:`, `<u>Bot Banner_Settings Commands</u>:` (+91 more)
...
- **Why does `AMPInstance` connect `AMPInstance` to `Banner`, `.__init__`, `WhitelistSync`, `Whitelist`, `.Login`, `AMP.py`, `Banner_Generator`, `botUtils`, `botEmbeds`, `Edited_DB_Banner`, `.__init__`, `.ConsoleMessage`, `.getStatus`, `.check_GatekeeperRole_Permissions`, `AMPConsole`, `Context`, `AMP_Tasks`, `.CurrentSessionHasPermission`, `.ConsoleMessage_withUpdate`?**
...
- **Why does `Database` connect `Database` to `DBConfig`, `DB.py`, `DB_Update`, `._fetchone`, `DBUser`, `._execute`, `DBServer`, `.AddServerDonatorRole`, `.AddServerWhitelistRole`?**
  _High betweenness centrality (0.091) - this node is a cross-community bridge._
- **Are the 12 inferred relationships involving `AMPInstance` (e.g. with `AMP_Tasks` and `AMPConsole`) actually correct?**
  _`AMPInstance` has 12 INFERRED edges - model-reasoned connections that need verification._
