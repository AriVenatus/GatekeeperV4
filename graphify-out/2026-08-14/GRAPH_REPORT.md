...
## Corpus Check
- 73 files · ~330,504 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1365 nodes · 2637 edges · 95 communities (64 shown, 31 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 76 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `6b08b953`
...
### Community 5 - "DB.py"
Cohesion: 0.24
Nodes (10): setup(), datetime, get_language(), getI18nHandler(), # NOTE: `Command._params` is a private/undocumented discord.py attribute…, retranslate_command_tree(), set_language(), t_plural() (+2 more)

### Community 6 - "Whitelist"
Cohesion: 0.09
Nodes (24): choices, AMPInstance, autocomplete, Bot, Choice, Context, hybrid_command, Interaction (+16 more)
...
### Community 11 - "AMP.py"
Cohesion: 0.19
Nodes (4): AMPCsgo, AMPCsgoConsole, AMPStarbound, AMPStarboundConsole
...
Cohesion: 0.09
Nodes (12): botUtils, Gatekeeper Utility Class, Formats the message for Discord `Bold = \\x01, \\x02` `Italic = \\x03, \\x04`…, Converts an IGN to a UUID/Name Table `returns 'uuid'` else returns `None`,…, Resolves a Minecraft in-game name into profile info via the official Mojang…, Converts a Steam Name to a Steam ID returns `STEAM_0:0:2806383`, Returns `True` if a Steam Web API Key has been set via GATEKEEPER_STEAM_API_KEY, Parses a raw SteamID64, a full profile URL (`/profiles/<id>` or… (+4 more)
...
Cohesion: 0.09
Nodes (11): DBServer, DB Server Attributes: `InstanceID: str` `InstanceName: str` `FriendlyName: str`…, Adds the provided RegexPattern ID/Name to the ServerRegexPatterns Table., Removes the provided RegexPattern ID/Name from the ServerRegexPatterns Table., Gets all Regex Patterns related to Server Returns `dict['ID': {'Name':…, Removes a Discord Role ID from this Server's Whitelist Role gate list., Gets all Discord Role IDs gating Whitelist access for this Server., Removes a Discord Role ID from this Server's Donator Role gate list. (+3 more)

### Community 26 - "Codebase Audit — 2026-08-14"
Cohesion: 0.18
Nodes (10): Codebase Audit — 2026-08-14, Conflicts / Notes Between Subagents, Critical Issues (ranked by severity), Delete/remove, Doc fixes, Flag, don't auto-delete (needs judgment), Quick Wins (safe, low-risk cleanup), Small correctness-adjacent fixes (harmless today, worth tightening) (+2 more)
...
### Community 29 - "Banner_Editor_View"
Cohesion: 0.16
...
Cohesion: 0.13
Nodes (11): discordBot, Message, Deletes `message`. Deleting another user's message requires the…, This will be used to access channel history up to 100. Simple scraper with…, Edits `message`; `content` must be convertible to `str`., Sends `content` to `parameter`. Only one of `file`/`files` may be given, not…, The name and ID of a custom emoji can be found with the client by prefixing…, This is the bot utils Channel Parse Function It handles finding the specificed… (+3 more)
...
### Community 36 - "Cancel_Banner_Button"
Cohesion: 0.13
...
Cohesion: 0.22
Nodes (6): Member, Adds `role` to `user`., Removes `role` from `user`., This is the bot utils Role Parse Function It handles finding the specificed…, This is the bot utils User Parse Function It handles finding the specificed…, Role
...
### Community 43 - ".user_role"
Cohesion: 0.29
Nodes (6): autocomplete, Context, hybrid_command, Member, User, User
...
Cohesion: 0.21
Nodes (7): AMPInstance, Context, Fills whitelist reply placeholders: `<user>`, `<server>`, `<guild>`., This checks the DB Server objects Avatar_url and returns the proper object…, This is the botUtils Server Parse function. **Note** Use context.guild.id…, Verifies if the AMP Server exists and if its Instance is running and its ADS is…, Use to get a Users Role Prefix for displaying.
...
Cohesion: 0.12
Nodes (15): Bot, Client, AMP_Tasks, Bot, Client, listener, loop, Message (+7 more)
...
### Community 79 - "Banner_Editor_Select"
Cohesion: 0.33
Nodes (5): banner_field_label(), Banner_Editor_Select, Interaction, Message, Select

## Knowledge Gaps
- **104 isolated node(s):** `Critical Issues (ranked by severity)`, `Delete/remove`, `Doc fixes`, `Small correctness-adjacent fixes (harmless today, worth tightening)`, `Flag, don't auto-delete (needs judgment)` (+99 more)
...
- **Why does `AMPInstance` connect `AMPInstance` to `Banner`, `.__init__`, `WhitelistSync`, `.Login`, `AMP.py`, `Banner_Generator`, `botEmbeds`, `Banner_Editor_View`, `.__init__`, `Cancel_Banner_Button`, `.getStatus`, `.check_GatekeeperRole_Permissions`, `AMPConsole`, `AMPHandler`, `AMP_Tasks`, `.CurrentSessionHasPermission`, `.ConsoleMessage_withUpdate`, `.RestartInstance`, `modal.py`, `Banner_Editor_Select`?**
  _High betweenness centrality (0.183) - this node is a cross-community bridge._
- **Why does `role_check()` connect `role_check` to `discordBot.py`, `Banner`, `WhitelistSync`, `DB.py`, `Whitelist`, `Cog_Template`, `.user_role`, `async_rolecheck`, `DB_User`, `Regex`, `DB_Server`?**
  _High betweenness centrality (0.133) - this node is a cross-community bridge._
- **Why does `Database` connect `Database` to `.AddServerDonatorRole`, `DBConfig`, `DB.py`, `DB_Update`, `._fetchone`, `DBUser`, `._execute`, `DBServer`, `.AddServerWhitelistRole`?**
...
- **What connects `Critical Issues (ranked by severity)`, `Delete/remove`, `Doc fixes` to the rest of the system?**
